...
public class LinkText : Text, IPointerClickHandler
{
    ...
    private class HyperlinkInfo
    {
        ...
        public List<int> linefeedIndexList = new();
    }
    ...
    /// 超链接信息列表
    private readonly List<HyperlinkInfo> m_HrefInfos = new();

    /// 文本构造器
    protected StringBuilder s_TextBuilder = new();

    [Tooltip("超链接文本颜色")] [SerializeField] private Color32 innerTextColor = new(36, 64, 180, 255);

    /// 超链接正则
    private static readonly Regex s_HrefRegex = new(@"<link=([^>\n\s]+)>(.*?)(</link>)", RegexOptions.Singleline);
    ...
    public string GetHyperlinkInfo => text;
